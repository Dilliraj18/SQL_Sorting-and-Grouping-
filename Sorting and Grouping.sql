...
BEGIN
    IF NEW.salary IS NULL THEN
        ...
        SET NEW.salary = IFNULL(@avg_salary, 0);
    END IF;
END //
...
CREATE TRIGGER retain_salary_before_update
...
CREATE TRIGGER prevent_delete_latest_year
...
BEGIN
    ...
    SET latest_year = (SELECT MAX(YEAR(hire_date)) FROM Employee);

    IF YEAR(OLD.hire_date) = latest_year THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot delete employees who joined in the latest year';
    END IF;
END //
